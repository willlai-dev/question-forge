...
> **目前狀態：全部完成（Phase 0～5）**——規格 §22 的 21 條驗收標準全數達成。
>
> 可建立帳號、管理題庫階層、匯入題庫、**實際刷題**（程式判分、錯題本、熟練狀態）、
> **用受控標籤組織題庫**，對錯題按下 **AI 深度解析**（三階段查證：規劃 → 搜尋外部資料 →
> 產生解析，附上實際查到的來源連結；AI 認為題庫答案有誤時只會建立待審爭議，
> **絕不自行修改答案**），以及 **整體學習診斷**——先由資料庫算出各維度正確率與趨勢，
> 再挑出最多 15 題代表性錯題交給 AI，找出跨知識點的共同錯誤模式與複習優先順序。
>
> 詳見 [實作計畫](./docs/IMPLEMENTATION_PLAN.md)。
...
| `/analysis/aggregate` | **學習診斷**：各維度正確率與趨勢、仍在連錯的知識點、AI 整合診斷 |
...
| `/settings` | 作答預設值、系統資訊（機密只顯示「已設定」）、Prompt 版本、維護作業 |
