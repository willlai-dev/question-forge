...
## Phase 5：多題分析與管理功能 ✅ 已完成

**目標**：從單題解析升級到整體學習診斷。

### 與原規劃的差異

進入 Phase 5 前重新盤點，原列的 8 項工作有 2 項在 Phase 4 就已交付，
另有 3 個既有缺陷會被統計層直接繼承，因此一併處理。

| 原項目 | 實際 |
|---|---|
| 1. 統計彙總 | ✅ `AggregateStatsService`，11 項統計 |
| 2. 代表錯題挑選 | ✅ 決定性純函式，最多 15 題 |
| 3. 多題分析 | ✅ 表 + schema + job + 端點 |
| 4. AI 任務管理 | **Phase 4 已交付**，本階段只補上多題任務的重跑路徑 |
| 5. Prompt 版本管理 | ✅ 唯讀清單（見下方決定） |
| 6. AI 用量頁面 | **Phase 4 已交付** |
| 7. 維護佇列 | ✅ 改為手動觸發端點（見下方決定） |
| 8. 前端 | ✅ `/analysis/aggregate`、`/settings` |

### 範圍決定

| 決定 | 理由 |
|---|---|
| `/settings` 只放作答預設值 + 唯讀系統資訊 | 規格只寫「系統設定頁」一行。收斂成改動不會破壞資料的項目；機密一律只顯示「已設定」 |
| Prompt 版本**唯讀**，不提供切換 | 執行期版本來自程式碼常數，且版本是 AI 快取鍵的一部分——切換等於讓既有解析全部失效 |
| 維護作業**手動觸發**，不排程 | 單機工具關機時排程不會執行，而會自行刪資料的背景程序風險高於效益。先預覽再執行 |
| 清理只刪「過期**且**無人引用」的網頁快取 | 被引用的來源即使過期也要留，否則既有解析的引用會指向不存在的東西（違反驗收 #16）。證據集合一律不刪 |
| `occurrence_count` 不回填歷史值 | 手動標記沒有留下指派歷史、真實次數無法還原；只回填 AI 來源會讓兩種來源失去可比性。一致的低估優於不一致的猜測 |

### 一併修掉的既有缺陷

| 缺陷 | 影響 |
|---|---|
| `StatsService.overview()` 未套用診斷判準 | 儀表板與錯題頁的錯題總數對不起來。判準抽成 `apps/api/src/common/diagnostic-scope.ts` 由三處共用 |
| `occurrence_count` 恆為 1 | 「各錯誤類型次數」會得到錯的數字。只讓 AI 路徑遞增，且以「這次分析確實是新的」為前提 |
| `/mistakes` 在交卷前洩漏答案 | `after_submit` 的防護只在 quiz-session 端點成立。連「出現在列表裡」都要擋——沒對答案前，一題出現在錯題本本身就等於告訴你答錯了 |

### 新增資料表

`aggregate_analyses`（migration `0004_phase5_aggregate_analysis`）。額外加了 ERD 未列的
`ai_job_id`（否則無法追溯分析出自哪次執行）、`stats_snapshot NOT NULL`、
代表錯題數量的 CHECK 約束，以及 `user_answers` 的期間查詢部分索引。

### 測試

- 單元：4 支純函式（percent／trend／streaks／representative）＋ 多題分析的語意驗證，共 +47 個測試
- E2E：`tests/api-e2e/phase5.mjs`，87 項。重點是**少掉任何一個過濾條件就會失敗**的斷言

### 驗收對照
規格 §22 之 19、21 —— 至此 21 條全數達成。
