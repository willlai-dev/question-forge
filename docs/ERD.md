...
> 這個約束是「AI 引用只能指向實際存在來源」（驗收標準 #16）的資料庫基礎。
>
> **實作採「整份拒絕」而非「逐筆剔除」**：引用是否都在本集合內，由
> `buildFinalExplanationSchema` 的 `superRefine` 在 Zod 驗證階段檢查，
> 不符就是整份輸出驗證失敗、進入有限次重生，重生仍失敗則該次分析失敗。
>
> 原本這裡寫的是「不符者剔除並將 `requires_human_review` 設為 true」——
> 程式碼裡沒有這條路徑。兩者都能滿足 #16，但剔除等於默默改動模型輸出、
> 留下一份「看起來完整、其實被動過手腳」的解析；整份拒絕則讓問題直接浮出來。
> 這裡改為記述實際行為。
