...
> 版本：0.1.0（Phase 0）；狀態欄位於 Phase 5 完成後逐條核對實作現況。

編號規則：`FR-<模組>-<序號>`。每條標註實作階段（P1～P5）與完成狀態。

| 標記 | 意義 |
|---|---|
| ✅ | 已實作。判準是「有對應程式碼**且**有會因它壞掉而失敗的測試」——不是「看起來有做」 |
| ⚠️ | 部分達成或刻意偏離規格。同一列會寫明實際行為與理由，不用模糊字眼帶過 |
| 無標記 | 不應該存在。若看到沒有標記的列，代表狀態未經核對，請當成「不確定」 |

目前 107 條中 104 條 ✅、3 條 ⚠️（FR-Q-09、FR-JOB-01、FR-JOB-06），沒有未標記的列。
驗證基礎：327 個單元測試與 449 項端到端檢查（全新資料庫），詳見 [TEST_PLAN](./TEST_PLAN.md)。
...
| FR-QUIZ-15 ✅ | 交卷時可選擇分數的分母：`all_questions`（未作答視同答錯，預設，模擬考算法）或 `answered_only`（只算實際作答的題數，時間不夠提早交卷時用）。選擇寫入 `quiz_sessions.scoring_mode`，日後重看結果頁數字一致。**兩種模式都不影響儀表板與學習診斷**——那些統計一律由 `user_answers` 推導，未作答的題目本來就不在那張表裡。正確率（accuracy）一律以已作答題數為分母，與此設定無關。 | P8 |
